...
// 基2迭代fft，时间抽取，无二进制逆序
void fft_radix2_dit_rec(Complex *input, size_t fft_len)
{
    // 小于某阈值调用迭代FFT
    if (fft_len <= (1 << 6))
    {
        fft_radix2_dit(input, fft_len, false);
        return;
    }
    const size_t half_len = fft_len / 2;
    // 先分别求前半部分和后半部分的DIT
    fft_radix2_dit_rec(input, half_len);
    fft_radix2_dit_rec(input + half_len, half_len);
...
// 基2递归fft，频率抽取，无二进制逆序
void fft_radix2_dif_rec(Complex *input, size_t fft_len)
{
    // 小于某阈值调用迭代FFT
    if (fft_len <= (1 << 6))
    {
        fft_radix2_dif(input, fft_len, false);
        return;
    }
    const size_t half_len = fft_len / 2;
    // 先分别求前半部分和后半部分的DIT
    ...
    fft_radix2_dif_rec(input, half_len);
    fft_radix2_dif_rec(input + half_len, half_len);
}
...
vector<T> poly_multiply(const vector<T> &in1, const vector<T> &in2)
{
    ...
    fft_radix2_dif_rec(fft_ary, fft_len);
    for (size_t i = 0; i < fft_len; i++)
    {
    ...
    fft_radix2_dit_rec(fft_ary, fft_len);
